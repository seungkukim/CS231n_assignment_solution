...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for i in range(2 * N):
            denom_left += torch.exp(sim(z_k, out[i]) / tau) if i != k else 0
            denom_right += torch.exp(sim(z_k_N, out[i]) / tau) if i != (k+N) else 0


        l_left = - torch.log(numerator / denom_left)
        l_right = - torch.log(numerator / denom_right)
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs = dot_prod / (torch.linalg.norm(out_left, dim=1) * torch.linalg.norm(out_right, dim=1))
...
def compute_sim_matrix(out):
    ...
    normalized_out = out / torch.linalg.norm(out, dim=1).unsqueeze(1)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    one_sided_pos_sim = sim_positive_pairs(out_left, out_right)
    pos_sim = torch.cat([one_sided_pos_sim, one_sided_pos_sim], dim=0)
