...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        processed_features = self.visual_projection(features).unsqueeze(1)
        processed_caption = self.positional_encoding(self.embedding(captions))
        
        tgt_mask = torch.tril(torch.ones(T, T))

        logits =  self.transformer(processed_caption, processed_features, tgt_mask)

        scores = self.output(logits)
