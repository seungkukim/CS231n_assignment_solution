...
def rnn_step_backward(dnext_h, cache):
    ...
    dnext_tan = (1 - next_h ** 2) * dnext_h

    dx = dnext_tan @ Wx.T
    dprev_h = dnext_tan @ Wh.T
    dWx = x.T @ dnext_tan
    dWh = prev_h.T @ dnext_tan
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        x_t = x[:,i,:]
        hidden = h[:,i-1,:] if i != 0 else h0
        out_t, cache_t = rnn_step_forward(x_t, hidden, Wx, Wh, b)
        h[:,i,:] = out_t
        cache[i] = cache_t
...
def rnn_backward(dh, cache):
    ...
    for i in range(T-1, -1, -1):
        dprev_h = np.zeros_like(dh0) if i == T-1 else dprev_h
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    input = sigmoid(activation[:,0:H])
    forget = sigmoid(activation[:,H:2*H])
    output = sigmoid(activation[:,2*H:3*H])
    block = np.tanh(activation[:,3*H:4*H])

    next_c = forget * prev_c + input * block
    next_h = output * np.tanh(next_c)

    cache = input, forget, output, block, next_c, next_h, prev_c, prev_h, x, Wx, Wh
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    input, forget, output, block, next_c, next_h, prev_c, prev_h, x, Wx, Wh = cache

    
    dc_sum = dnext_c + dnext_h * output * (1 - np.tanh(next_c) ** 2)

    dprev_c = dc_sum * forget
    dforget = dc_sum * prev_c
    dinput = dc_sum * block
    dblock = dc_sum * input
    dout = dnext_h * np.tanh(next_c)

    # Non-linearity
    dinput = dinput * input * (1 - input)
    dforget = dforget * forget * (1 - forget)
    dout = dout * output * (1 - output)
    dblock = dblock * (1 - block ** 2)
    dtgt = np.concatenate((dinput, dforget, dout, dblock), axis=1)

    dWx = x.T @ dtgt
    dWh = prev_h.T @ dtgt
    dx = dtgt @ Wx.T
    dprev_h = dtgt @ Wh.T
    db = np.sum(dtgt, axis=0)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        x_t = x[:,i,:]
        hidden = h[:,i-1,:] if i != 0 else h0
        cellstate = c[:,i-1,:] if i != 0 else c0
        out_h, out_c, cache_t = lstm_step_forward(x_t, hidden, cellstate, Wx, Wh, b)
        h[:,i,:] = out_h
        c[:,i,:] = out_c
        cache[i] = cache_t
...
def lstm_backward(dh, cache):
    ...
    dx, dh0 = np.zeros((N, T, D)), np.zeros((N, H))
    dWx, dWh = np.zeros((D, 4*H)), np.zeros((H, 4*H))
    db = np.zeros(4*H)

    for i in range(T-1, -1, -1):
        dprev_h = np.zeros_like(dh0) if i == T-1 else dprev_h
        dprev_c = np.zeros_like(dh0) if i == T-1 else dprev_c
        dx_t, dprev_h_t, dprev_c_t, dWx_t, dWh_t, db_t = lstm_step_backward(dh[:,i,:] + dprev_h, dprev_c, cache[i])
